...
//            json obj1 ;
//            obj1["source"] = obj["source"];
//            obj1["destination"] = obj["destination"];
//            auto edgePropertyJsonTemp = obj["properties"];
//            edgePropertyJsonTemp["graphId"] = obj["source"]["properties"]["graphId"];
//            edgePropertyJsonTemp["pid"] = temp_s;
//            obj1["properties"] = edgePropertyJsonTemp;
            ...
            workerClients.at((int) temp_s)->publish_relation(obj1.dump());
            workerClients.at((int) temp_d)->publish_relation(obj2.dump());
        }
    }
...
void listen_to_kafka_topic_1(KafkaConnector *kstream, Partitioner &graphPartitioner, vector<DataPublisher *> &workerClients,
                        int noOfPartitions) {
    bool native_store = true;
//    std::ofstream outfile("/home/chandima/ubuntu/output.csv");  // Step 1

    std::vector<ofstream> edgePartitions(noOfPartitions);
    std::vector<ofstream> nodePartitions(noOfPartitions);

    for (int i = 0; i < noOfPartitions; ++i) {
        edgePartitions[i] = std::ofstream(
                "/home/sandaruwan/ubuntu/software/fyp-datasets/partitioned_" + std::to_string(noOfPartitions) +
                "/dblp.v14-co-author_edges_" + std::to_string(noOfPartitions) + "_" + std::to_string(i) + ".csv");
        if (!edgePartitions[i].is_open()) {
            std::cerr << "Error: Cannot open edge output file.\n";
        }
        edgePartitions[i] << "source,target,timestamp,weight" << std::endl;
    }

    std::string filename = "/home/sandaruwan/Downloads/dblp-co-author/co_author_edgelist.csv";
        ...
        if (count > 0) {
            ...
            for (int i = 1; i <= 3; i++) {
                if (std::getline(lineStream, cell, ',')) {
                    if (i == 3) {
                        // Remove spaces from the data in column 3
                        cell.erase(std::remove_if(cell.begin(), cell.end(), [](char c) { return std::isspace(c); }),
                                   cell.end());
                    }
                    if (i == 1) {
                        source_id = cell;
            ...
            partitionedEdge partitionedEdge = graphPartitioner.addEdge({source_id, destination_id});
            ...
            if (temp_s == temp_d) {
                edgePartitions[temp_s] << source_id << "," << destination_id << "," << timestamp << "," << weight
                                       << std::endl;
            } else {
                edgePartitions[temp_s] << source_id << "," << destination_id << "," << timestamp << "," << weight
                                       << std::endl;
                edgePartitions[temp_d] << source_id << "," << destination_id << "," << timestamp << "," << weight
                                       << std::endl;
            }
        }
        count++;
    ...
    for (int k = 0; k < noOfPartitions; ++k) {
        edgePartitions[k].close(); //datasetname_edges_partitioncount_partitionid
    }
...
//    Partitioner graphPartitioner(numberOfPartitions, 1, spt::Algorithms::HASH);
    ...
    while (!loop) {
        ...
        } else if (line.compare(SAVE_STREAM_KAFKA) == 0) {
            Utils utils;
...
//          create kafka consumer and graph partitioner
            ...
            std::vector<ofstream> edgePartitions(numberOfPartitions);

            for (int i = 0; i < numberOfPartitions; ++i) {
                edgePartitions[i] = std::ofstream("/home/sandaruwan/ubuntu/edge_" + std::to_string(i) + ".csv");
                if (!edgePartitions[i].is_open()) {
                    std::cerr << "Error: Cannot open output file.\n";
                }
                edgePartitions[i] << "source,destination,timestamp,weight" << std::endl;
            }
            ...
            while (true) {
                ...
                std::string sId = std::string(edgeJson["source"]);
                std::string dId = std::string(edgeJson["destination"]);
                partitionedEdge partitionedEdge = graphPartitioner.addEdge({sId, dId});

                string edge = edgeJson.dump();
                ...
                if (temp_s == temp_d) {
                    edgeJson["pid"] = temp_s;
                    workerClients.at((int) temp_s)->publish_relation(edgeJson.dump());

                    edgePartitions[temp_s] << edgeJson["source"] << "," << edgeJson["destination"] << ","
                                           << edgeJson["timestamp"] << "," << edgeJson["weight"] << std::endl;
                } else {
                    auto edgeJson1 = edgeJson;
                    edgeJson1["pid"] = temp_s;
                    workerClients.at((int) temp_s)->publish_relation(edgeJson1.dump());

                    edgePartitions[temp_s] << edgeJson["source"] << "," << edgeJson["destination"] << ","
                                           << edgeJson["timestamp"] << "," << edgeJson["weight"] << std::endl;

                    auto edgeJson2 = edgeJson;
                    edgeJson2["pid"] = temp_d;
                    workerClients.at((int) temp_d)->publish_relation(edgeJson2.dump());

                    edgePartitions[temp_d] << edgeJson["source"] << "," << edgeJson["destination"] << ","
                                           << edgeJson["timestamp"] << "," << edgeJson["weight"] << std::endl;
                }
                count++;
                cout << count << endl;
...
//          create kafka consumer and graph partitioner
            ...
            input_stream_handler = thread(listen_to_kafka_topic, kstream, std::ref(graphPartitioner),
                                          std::ref(workerClients), numberOfPartitions);
